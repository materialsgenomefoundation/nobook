...
from functools import lru_cache, partial, partialmethod, singledispatch, wraps
...
def _is_async(x: Series | DataFrame | pandas.core.groupby.DataFrameGroupBy):
    ...
    if len(x):
        return isinstance(
            x.iloc[0], (AsyncGenerator, AsyncIterable, AsyncIterator, Coroutine)
        )
    return False
...
class _itemgetter(Accessor, types=INDEX | SERIES, name="itemgetter"):
    ...
    def _itemgetter(self, f, object, default=None):
        try:
            return f(object)
        except:
            return default

    def __call__(self, *args, default=None, **kwargs):
        from operator import itemgetter

        return apply(
            self.object,
            partial(self._itemgetter, itemgetter(*args, **kwargs), default=default),
        )
...
class _enumerate(Accessor, types=SERIES, name="enumerate"):
    ...
    def __call__(self, name=None, start=0):
        ...
        out = object.groupby(object.index).apply(
            lambda s: (
                Series(
                    s.values, RangeIndex(start=start, stop=start + len(s), name=name)
                )
                if isinstance(s, Series)
                else s.set_index(
                    RangeIndex(start=start, stop=start + len(s), name=name)
                )
            )
        )
...
class _gather(Accessor, types=SERIES, name="gather"):
    async def __call__(self, *args, **kwargs):
        return pandas.Series(
            await asyncio.gather(*self.object), self.object.index, *args, **kwargs
        )
...
async def get__file_index(
    ...
    return [
        ...
        async for path in get__file_index_iter(
            parent, include, exclude, recursive, root=root
        )
    ]
...
def mime_pipe(caller):
    caller.LOADERS = dict()

    def wrapped(object, mime):
        return caller.LOADERS.get(mime, caller)(object)

    def _register_mime(*mimes):
        def _complete_registration(callable):
            for mime in mimes:
                caller.LOADERS[mime] = callable
            return callable

        return _complete_registration

    wrapped.register = _register_mime
    return wrapped
...
@loads.register("application/x-ipynb+json", "application/json")
def loads_json(x):
...
@mime_pipe
def get_notebook(x):
    from nbformat.v4 import new_notebook, new_raw_cell

    if isinstance(x, str):
        return new_notebook(cells=[new_raw_cell(x)])
    return new_notebook(cells=[new_raw_cell("", metadata=x)])


@get_notebook.register("application/x-ipynb+json")
def get_notebook_functor(x):
    from nbformat import from_dict

    return from_dict(x)


@get_notebook.register("text/markdown")
def get_notebook_md(x):
    from nbformat.v4 import new_markdown_cell, new_notebook

    return new_notebook(cells=[new_markdown_cell(x)])


@get_notebook.register("text/x-python")
def get_notebook_py(x):
    from nbformat.v4 import new_code_cell, new_notebook

    return new_notebook(cells=[new_code_cell(x)])
...
class Path(type(Path())):
    def load(self):
        try:
            ...
            raise ValueError(f"failed to load {self}")
...
class APath(anyio.Path):
    async def load(self, sha=False, key="contents"):
        bytes = await self.read_bytes()
        try:
            data = loads(bytes.decode(), APath.mime(self))
        except BaseException:
            raise ValueError(f"failed to load {self}")
        if sha:
            return {"sha": sha256(bytes).hexdigest(), key: data}
        return data

    async def notebook(self, sha=True, key="contents"):
        data = await APath.load(self, sha=sha, key=key)
        if sha:
            data.update({key: get_notebook(data[key], APath.mime(self))})
            return data
        return get_notebook(data, APath.mime(self))
    ...
    async def as_uri(self):
        from anyio import Path

        return Path.as_uri(await self.absolute())
    ...
    def __call__(self, pattern, **kwargs):
        ...
        return pandas.concat(
            [
                dir
                / Series(
                    patterns := glob(pattern, root_dir=dir, **kwargs),
                    index=[dir] * len(patterns),
                    name="file",
                )
                for dir in self.object
            ]
...
class _APath(Method, method=APath, types=INDEX | SERIES, name="apath"):
    ...
    async def notebook(self, sha=True, key="contents"):
        data = await self.apply(APath.notebook, sha=sha, key=key)
        if sha:
            return data.series()
        return data
...
if "IPython" in sys.modules:
    ...
    if get_ipython():
        ...
        class _display(Method, types=INDEX | SERIES, name="display", method=display):
            def __call__(self, *args, **kwargs):
                display.display(*self.object, *args, **kwargs)

            def iframe(self, height=600, width="100%"):
                from html import escape

                return self.object.apply(
                    lambda x: display.HTML(
                        f"""<iframe srcdoc="{escape(x)}" height="{height}" width={width}/>"""
                    )
                )

        # we probably need some dispatching on paths for files or urls
        # for this to work consistently with the current api
...
class _http(Accessor, types=SERIES | INDEX, name="http"):
    def __init__(self, object):
        ...
        self._client_cache = partial(
            ...
            cache=aiohttp_client_cache.SQLiteBackend(".pandas.cache.sqlite"),
        )
