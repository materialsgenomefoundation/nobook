...
class _attrgetter(Accessor, types=INDEX | SERIES, name="attrgetter"):
    def __call__(self, *args, **kwargs):
        return apply(self.object, attrgetter(*args, **kwargs))
...
class _itemgetter(Accessor, types=INDEX | SERIES, name="itemgetter"):
    def __call__(self, *args, **kwargs):
        ...
        return apply(self.object, itemgetter(*args, **kwargs))


class explode_index(Accessor, types=SERIES, name="explode_index"):
    def __call__(self, *args, **kwargs):
        if isinstance(self.object, pandas.Series):
            return pandas.concat({k: pandas.Series(v) for k, v in self.object.items()})
        elif isin(self.object, pandas.DataFrame):
            return
        raise NotImplemented()


class _aseries(Accessor, types=SERIES, name="aseries"):
    def __call__(self, *args, **kwargs):
...
def get_environment():
    ...
    env = jinja2.Environment(
        ...
        extensions=["jinja2.ext.loopcontrols", "jinja2.ext.with_", "jinja2.ext.do"],
    )
...
async def get__file_index(
    ...
    return [
        ...
        async for path in get__file_index_iter(
            anyio.Path(path), include, exclude, recursive
        )
    ]
    ...
    if await path.is_dir():
        async for path in path.iterdir():
            if await path.is_file():
                ...
                async for path in get__file_index_iter(
                    path, include, exclude, recursive
                ):
                    yield path
...
def loads_toml(x):
    return __import__("tomli").loads(x.read_text())


@loads.register("application/x-ipynb+json")
@loads.register("application/json")
def loads_json(x):
    data = __import__("json").loads(x.read_text())
...
def loads_yaml(x):
    try:
        return __import__("ruamel.yaml").yaml.safe_load(x.read_text())
    except ModuleNotFoundError:
        return __import__("yaml").safe_load(x.read_text())


@loads.register("text/ini")
def loads_ini(x):
    parser = ConfigParser()
    parser.read_string(x.read_text())
